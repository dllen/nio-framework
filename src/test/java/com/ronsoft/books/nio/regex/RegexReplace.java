...
/**
 * Exercise the replacement capabilities of the java.util.regex.Matcher class. Run this code from the command line with three or more arguments. 1) First argument is a regular expression 2) Second
 * argument is a replacement string, optionally with capture group references ($1, $2, etc) 3) Any remaining arguments are treated as input strings to which the regular expression and replacement
 * strings will be applied. The effect of calling replaceFirst() and replaceAll() for each input string will be listed.
 * <p>
