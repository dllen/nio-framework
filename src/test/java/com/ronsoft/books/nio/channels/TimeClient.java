...
/**
 * Request time service, per RFC 868.  RFC 868 (http://www.ietf.org/rfc/rfc0868.txt) is a very simple time protocol whereby one system can request the current time from another system. Most Linux, BSD
 * and Solaris systems provide RFC 868 time service on port 37.  This simple program will inter-operate with those. The National Institute of Standards and Technology (NIST) operates a public time
 ...
 * by adding or subtracting 2,208,988,800 as appropriate.  To avoid shifting and masking, a four-byte slice of an eight-byte buffer is used to send/recieve.  But getLong() is done on the full eight
 * bytes to get a long value.
 * <p>
 * When run, this program will issue time requests to each hostname given on the command line, then enter a loop to receive packets. Note that some requests or replies may be lost, which means this
