...
/**
 * Request and/or provide time service, per RFC 868.  RFC 868 (http://www.ietf.org/rfc/rfc0868.txt) is a very simple time protocol whereby one system can request the current time from another system.
 * It's a very simple protocol, specified nearly 20 years ago, which does not take into account packet transit time or precision of less than one second.  The Network Time Protocol (NTP) is superior
 * but far too complex for demonstration purposes. Most Linux, BSD and Solaris systems provide RFC 868 time service on port 37.  This simple program will inter-operate with those. The National
 * Institute of Standards and Technology (NIST) operates a public time server at time.nist.gov.
 * <p>
 * This code also implements an RFC 868 listener to provide time service.  On most unix systems, root privileges are required to bind to ports below 1024.  You can either run this code as root or
 * provide another port number with "-l port#".  If you run this server on an alternate port, you can connect from another JVM by supplying the "-p" option to specify the alternate port for requests.
 * <p>
 * Note: The familiar rdate command on unix will probably not work with this server.  Most versions of rdate use TCP rather than UDP to request the time.
 * <p>
 * When run, this program will issue time requests to each hostname given on the command line, then enter a loop to receive packets. If not acting as a server, it will exit when all expected replies
 * have arrived.  Note that some replies may be lost, which means this code may block forever.  If acting as a server, time requests from remote systems will be dispatched as well, in the same loop.
 * <p>
 * The RFC 868 protocol specifies a 32 bit unsigned value be sent, representing the number of seconds since Jan 1, 1900.  The Java epoch begins on Jan 1, 1970 (same as unix) so an adjustment is made
 * by adding or subtracting 2,208,988,800 as appropriate.  To avoid casting shifting an masking, a four-byte slice of an eight-byte buffer is used send/recieve, but then get/putLong() is done on the
